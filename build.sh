...
go build -ldflags "-w -X main.Version=${GIT_TAG} -X main.BuildTime=${BTIME}" -o ./build/bin/tl
tar -czvf ./build/linux_X86-64.tar.gz ./build/bin
# Build for Mac OSX
env GOOS=darwin GOARCH=amd64 go build -ldflags "-w -X main.Version=${GIT_TAG} -X main.BuildTime=${BTIME}" -o ./build/bin/tl
